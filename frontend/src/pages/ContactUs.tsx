            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="flex flex-col space-y-2">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <a href={siteConfig.github}><Github className="w-6 h-6 text-white" /></a>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants}>
            <form onSubmit={handleSubmit} className="space-y-5 alegreya-sans-sc-regular">
              ...
              <motion.div variants={itemVariants}>
                ...
                <input type="tel" id="tel" name="tel" value={formData.tel} onChange={handleChange} placeholder="Your Phone Number"
                  className="w-full bg-slate-800/70 border border-slate-700 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all duration-300 placeholder:text-slate-500" />
              </motion.div>
              <motion.div variants={itemVariants}>
                <label htmlFor="subject" className="">Subject</label>
                <input type="text" id="subject" name="subject" value={formData.subject} onChange={handleChange} placeholder="Subject of your message"
                  className="w-full bg-slate-800/70 border border-slate-700 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all duration-300 placeholder:text-slate-500" />
              </motion.div>
              <motion.div variants={itemVariants}>
              ...
              <motion.div variants={itemVariants} className="flex flex-col items-center">
                <div className="text-center text-sm text-gray-400 p-2">
                  <Magnet padding={500} disabled={false} magnetStrength={10}>
                    <motion.button type="submit" disabled={!formData.agree || isSubmitting}
                      whileHover={{ scale: !isSubmitting ? 1.05 : 1, boxShadow: "0px 0px 15px rgba(124, 58, 237, 0.5)" }}
                      whileTap={{ scale: !isSubmitting ? 0.95 : 1 }}
                      transition={{ type: "spring", stiffness: 300 }}
                      className="w-full px-6 py-4 rounded-lg bg-gradient-to-r from-blue-400 to-blue-500 font-bold text-white hover:from-blue-500 hover:to-blue-600 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                      {isSubmitting && (
